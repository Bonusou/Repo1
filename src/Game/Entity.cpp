...
{
	auto* clone = new Entity();

	clone->mDirection = mDirection;
	clone->mSpeed = mSpeed;
	clone->mWidth = mWidth;
	clone->mHeight = mHeight;
	clone->mDefaultWidth = mDefaultWidth;
	clone->mDefaultHeight = mDefaultHeight;
	clone->mHitboxWidth = mHitboxWidth;
	clone->mHitboxHeight = mHitboxHeight;
	...
	clone->dirY = dirY;
	clone->timer = timer;
	clone->timerAnnim = timerAnnim;
	clone->mToDestroy = mToDestroy;
	clone->mTag = mTag;
	clone->Layout = Layout;
	clone->mLife = mLife;
	clone->mIsAlive = mIsAlive;
	clone->mHasbeenHit = mHasbeenHit;
	clone->hasFixedLayout = hasFixedLayout;

	clone->SetPosition(GetPosition().x, GetPosition().y);
	clone->SetScale(mSprite.getGlobalBounds().width, mSprite.getGlobalBounds().height);

	clone->mSprite = mSprite;
	clone->mTexture = mTexture; // utile si tu le gères toi-même
	clone->mSprite.setTexture(mTexture); // essentiel pour afficher correctement

	clone->mShape = mShape;

	clone->lSprite = lSprite;

	if (this->mSprite.getTexture()) {
		clone->mSprite.setTexture(*this->mSprite.getTexture());
	}

	// Recréer le bon type de hitbox
	if (auto* circle = dynamic_cast<CircleCollider*>(mHitbox)) {
		clone->AddCircleHitbox();
		clone->SetHitboxSize(circle->radius);
	}
	else if (auto* aabb = dynamic_cast<AABBCollider*>(mHitbox)) {
		clone->AddAABBHitbox();
		clone->SetHitboxSize(mHitboxWidth, mHitboxHeight);
	}

	clone->mSprite.setTextureRect(this->mSprite.getTextureRect());
	clone->haveHitbox = haveHitbox;
