...
void GameManager::SetPointer(Pointer* _pointer)
{
	SPointer = _pointer;
}
...
void GameManager::Save() {
	// Supprimer toute sauvegarde précédente
	for (auto e : mEntitiesSaved)
		delete e;
	mEntitiesSaved.clear();

	// Créer une nouvelle copie des entités
	for (Entity* entity : mEntities) {
		// On ne copie que les entités vivantes et pertinentes
		if (entity && entity->mIsAlive) {
			Entity* copy = entity->Clone(); // <-- nécessite que Entity ait une méthode Clone()
			copy->SetHide(true); // on les cache pour éviter qu'elles interfèrent
			mEntitiesSaved.push_back(copy);
		}
	}

	HaveSaved = true;
	std::cout << "Game saved. Entities saved: " << mEntitiesSaved.size() << std::endl;
}
...
void GameManager::Load() {
	...
	// Supprimer toutes les entités actuelles
	for (Entity* entity : mEntities) {
		delete entity;
	}
	mEntities.clear();

	// Restaurer les entités sauvegardées
	for (Entity* saved : mEntitiesSaved) {
		Entity* copy = saved->Clone(); // clone pour éviter double pointeur
		copy->SetHide(false);
		mEntities.push_back(copy);
	}
	...
	HaveSaved = false;
	std::cout << "Game loaded. Entities restored: " << mEntities.size() << std::endl;
}


bool GameManager::HaveSave() {
	return HaveSaved;
}
